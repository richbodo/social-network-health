...
D=research/research_library/paper-corpus/resolver.db
O=research/research_library/paper-corpus/pdfs
...
- **Corpus db:** `research/research_library/paper-corpus/resolver.db` (the SQLite cache *and* corpus index — reruns are
  cheap/idempotent). The whole `research/research_library/paper-corpus/` dir is gitignored (generated artifacts).
- **Downloads (when used):** plan to use `--download --extract --out research/research_library/paper-corpus/pdfs`.
...
- **Downloader hardened (2026-06-10, ported to this copy 2026-07-08):** PDF download now uses a
  browser User-Agent + a `%PDF` content check (so HTML landing/error pages aren't saved as fake
  PDFs). This recovered several UA-blocked publishers — BUT **SAGE, Wiley, Elsevier, ACM, INFORMS,
  and MDPI (ijerph) still block** scripted OA-PDF fetch (cookies/JS); those resolve to a full-text
  URL but need a real browser or manual pickup. Failed downloads stay queued (status=`fulltext`,
  `pdf_path` empty) — re-runnable later via `batch <failed-dois> --refresh --download`. A
  headless-browser fetch tier is the next upgrade if we want those.
