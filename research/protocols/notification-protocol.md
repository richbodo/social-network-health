...
**Status:** v0.1 (2026-07-04) — idea-stage design, workshopped for the DWeb Camp Berlin 2026
talk. Grill log: `brainstorms/20260704-notification-protocol-grill.md` (gitignored).
...
**A super contact exists only by community consent — the opt-in ceremony.** A steward
...
*(companion PNA feature — not protocol machinery; unsolved; spin out into its own note when
it grows)*
...
## Threat model (seed — full pass required before any experiment)
...
## Placement in the three-step plan

The talk plan's step 3 is renamed from "Build social network health analysis" to
**"Build community health analysis — and act on it, safely."** Internally:

1. **Analyze** — SNH analysis from egocentric data (the explainer).
2. **Act** — interventions, of which this protocol is the first.
3. **The threat-model gate** — the explicit condition for crossing from 1 to 2. Per the
   threat catalog's dependency order, nothing past guardrails is legitimate until the
   guardrails exist. Egocentric-audience asks are ordinary communication (the risk a user
   already chooses when they message a friend); super-contact asks are the danger zone and
   get the full staged machinery.
