...
**Status:** v0.1 (2026-07-08) — proposed protocol, idea-stage. Proposed by Rich out of the
project's memoriams postvention practice (memoriam post below); drafted with corpus evidence by the snhdb research agent. Open
...
A project leader's public memoriam post (2021), written after the death — believed
suicide — of a young prolific contributor to the project.
...
the member had gone quiet and stopped answering emails — was only acted on *after* another
...
Corpus copy: (reference withheld to match the memoriams anonymization posture).
...
2. **Absence-triggered** — the recurring memoriams pattern. If a member's ordinary visible activity
...
3. **Event-triggered** — after known acute stressors: a heated conflict/fork, a brutal
   release crunch, a public failure, a departure or layoff nearby. Named in the memoriam's
...
rescue. The community acted correctly with what it had; it just had no structure
that acted earlier.
...
1. **Scope of v1:** open-source projects (the memoriams context) or remote teams (SNH's core
